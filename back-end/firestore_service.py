...
#         else:
#             # In Cloud Run (GCP) use default credentials
...
#     #     # Create Firebase Auth user (sets password)
#     #     user = auth.create_user(email=email, password=password, display_name=username)
#     #     # Store user doc
#     #     data = {"username": username, "email": email, "uid": user.uid, "created_at": datetime.utcnow().isoformat()}
#     #     self.db.collection("users").document(user.uid).set(data)
#     #     return {"uid": user.uid, **data}
    
#     def create_user(self, username, email, password):
#         print("📩 Creating Firebase user:", email)
#         try:
#             if not password or len(password) < 6:
#                 raise ValueError("Password must be at least 6 characters long.")

#             auth_user = auth.create_user(email=email, password=password)
#             print("✅ Firebase Auth user created:", auth_user.uid)
...
#             print("✅ Firestore document created for:", auth_user.uid)
#             return {"uid": auth_user.uid}
...
#     # Auth token verification (for endpoints that require user login)
